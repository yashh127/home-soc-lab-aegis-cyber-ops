...
export default function ThreatMap({ activeAttacks, showHelp }) {
  ...
  const socTarget = { name: 'FALCON SENTINEL HUB', x: 50, y: 50, ip: '127.0.0.1 (Protected SIEM)' };

  return (
    <div className="cyber-card p-5 flex flex-col h-full border-red-500/20">
      {/* Header */}
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-red-500/20">
        ...
        <span className="badge-label bg-red-500/15 border border-red-500/30 text-red-300">
      ...
      {showHelp && (
        <div className="mb-3 text-xs bg-red-950/40 p-2.5 rounded-xl border border-red-500/30 text-red-200">
      ...
      <div className="relative w-full h-[320px] bg-slate-950/90 rounded-xl border border-slate-800 overflow-hidden flex items-center justify-center shadow-inner">
        ...
        {defaultNodes.map((node, idx) => (
          ...
          >
            <div className="relative">
              <div className="w-4 h-4 rounded-full bg-red-500 animate-ping opacity-75"></div>
              <div className="absolute inset-0 w-4 h-4 rounded-full bg-red-600 border-2 border-white shadow-lg shadow-red-500/50"></div>
            </div>

            {/* Tooltip on Hover */}
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 hidden group-hover:block z-30 w-52 p-3 rounded-xl bg-slate-900/95 border border-red-500/40 text-xs font-sans text-slate-200 shadow-2xl backdrop-blur-md pointer-events-none">
              <div className="flex items-center justify-between font-bold text-red-400 mb-1">
                <span>{node.name}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-red-500/20 text-red-300 font-mono">{node.risk}</span>
              </div>
              <div className="text-slate-300 font-mono text-[11px]">IP: {node.ip}</div>
              <div className="text-rose-300 mt-1 font-medium">{node.type}</div>
        ...
        >
          <div className="relative flex items-center justify-center">
            <div className="w-10 h-10 rounded-full bg-red-500/25 border-2 border-red-500 animate-pulse shadow-lg shadow-red-500/50"></div>
            <div className="absolute w-5 h-5 rounded-full bg-red-500 flex items-center justify-center text-white">
              <ShieldCheck className="w-3.5 h-3.5 text-white font-bold" />
            </div>
          </div>
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-1.5 text-[11px] font-mono font-bold text-red-200 whitespace-nowrap bg-slate-950/90 px-2.5 py-1 rounded-full border border-red-500/40 shadow-lg">
            {socTarget.name}
          </div>
        </div>
      </div>
