...
export default function ThreatMap({ activeAttacks, showHelp }) {
  ...
  return (
    <div className="cyber-card p-5 flex flex-col h-full border-zinc-700/80">
      ...
      <div className="flex items-center justify-between mb-3 pb-3 border-b border-zinc-700/80">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-300 font-medium">Visualizing attack origin locations targeting your SOC Sentinel Hub</p>
          </div>
        </div>
      ...
      {/* Map Canvas Box */}
      <div className="relative w-full h-[320px] bg-[#0e0f15] rounded-xl border border-zinc-700/90 overflow-hidden flex items-center justify-center shadow-inner">
        {/* Grid Dots */}
        <div 
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none">
          <defs>
            <linearGradient id="attackGrad" x1="0%" y1="0%" x2="100%" y2="100%">
              ...
              <stop offset="100%" stopColor="#e4e4e7" stopOpacity="0.45" />
            </linearGradient>
          </defs>
        ...
        {defaultNodes.map((node, idx) => (
          ...
          >
            <div className="relative">
              <div className="w-4 h-4 rounded-full bg-rose-500 animate-ping opacity-80"></div>
              <div className="absolute inset-0 w-4 h-4 rounded-full bg-rose-600 border-2 border-white shadow-lg shadow-rose-500/60"></div>
            </div>

            {/* Tooltip on Hover */}
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-3 hidden group-hover:block z-30 w-52 p-3 rounded-xl bg-zinc-800/98 border border-white/20 text-xs font-sans text-zinc-100 shadow-2xl backdrop-blur-md pointer-events-none">
              <div className="flex items-center justify-between font-bold text-white mb-1">
                <span>{node.name}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-rose-500/25 text-rose-300 font-mono font-bold">{node.risk}</span>
              </div>
              <div className="text-zinc-200 font-mono text-[11px]">IP: {node.ip}</div>
              <div className="text-zinc-300 mt-1 font-medium">{node.type}</div>
        ...
        >
          <div className="relative flex items-center justify-center">
            <div className="w-11 h-11 rounded-full bg-white/20 border-2 border-white animate-pulse shadow-xl shadow-white/30"></div>
            <div className="absolute w-5 h-5 rounded-full bg-white flex items-center justify-center text-zinc-950 shadow-md">
              <ShieldCheck className="w-3.5 h-3.5 text-zinc-950 font-bold" />
            </div>
          </div>
          <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-1.5 text-[11px] font-mono font-bold text-white whitespace-nowrap bg-zinc-800/95 px-3 py-1 rounded-full border border-zinc-600 shadow-xl">
            {socTarget.name}
          </div>
        </div>
      </div>
      ...
      {/* Legend / Footer */}
      <div className="mt-3.5 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-medium text-zinc-200">
        <div className="flex items-center gap-2 bg-zinc-800/90 px-3 py-2 rounded-xl border border-zinc-700">
