  ...
  return (
    <div className="cyber-card p-5 flex flex-col h-full border-red-500/20">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3.5">
        <div className="bg-slate-950/80 p-3.5 rounded-xl border border-red-500/30 flex flex-col items-center justify-center text-center relative overflow-hidden">
          <div className="text-[10px] font-mono font-semibold text-slate-400 uppercase tracking-wider mb-1">
            THREAT RISK INDEX
          </div>
          <div className="text-3xl font-extrabold font-cyber text-red-400 my-0.5">
            78<span className="text-xs text-slate-500 font-mono">/100</span>
          </div>
          <div className="text-[11px] text-red-300 font-medium flex items-center gap-1 mt-1">
      ...
      <div className="space-y-2 flex-1 overflow-y-auto">
        {advisories.map((adv) => (
          <div
            key={adv.id}
            className="p-2.5 rounded-xl bg-slate-950/90 border border-slate-800/90 hover:border-red-500/40 transition-colors flex items-center justify-between gap-3"
          >
            <div className="flex items-center gap-2.5 min-w-0">
              <AlertCircle className={`w-4 h-4 shrink-0 ${
                adv.severity === 'CRITICAL' ? 'text-red-400' : adv.severity === 'HIGH' ? 'text-amber-400' : 'text-slate-400'
              }`} />
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
                adv.severity === 'CRITICAL' ? 'bg-red-500/20 text-red-300 border border-red-500/30' :
